		...
		<table width="97%" class="WSY_table WSY_t2" id="WSY_t1">
			<thead class="WSY_table_header">
				<tr>
					...
					<th width="8%" nowrap="nowrap">预约门店</th>		
			...
			<tbody>
				...
				while ($row2 = mysql_fetch_object($result2)) {
	             ...
	             while ($row = mysql_fetch_object($result)) {				 
					...
					/*查询订单完成*/
					$query2="select totalprice,batchcode from weixin_commonshop_orders wco where  status=1 and  wco.isvalid=true and wco.user_id=".$user_id;
					$result2 = mysql_query($query2) or die('Query failed: ' . mysql_error());
					
				    while ($row2 = mysql_fetch_object($result2)) {
					   $totalprice_1 = $row2->totalprice;
					   $o_batchcode= $row2->batchcode;
						...
						/* 查询运费End */	
							$totalprice_1_last = $totalprice_1 + $o_express_price;	
							$totalprice_1_sum  +=$totalprice_1_last;
					}					
					/*查询订单完成end*/
					...
					$totalprice_wzf=0;
					$batchcode_wzf=-1;
					$query2="select totalprice,batchcode from weixin_commonshop_orders  where  paystatus=0 and  isvalid=true and user_id=".$user_id;
					$result2 = mysql_query($query2) or die('Query failed: ' . mysql_error());
					while ($row2 = mysql_fetch_object($result2)) {
					   ...
					   $batchcode_wzf= $row2->batchcode;
						  ...
						  //查询改价表
						$query2_2="select totalprice from weixin_commonshop_changeprices  where  status=1 and  isvalid=true and batchcode=".$batchcode_wzf;
						$result2_2 = mysql_query($query2_2) or die('Query failed: ' . mysql_error());
						while ($row2_2 = mysql_fetch_object($result2_2)) {
						   $totalprice_wzf= $row2_2->totalprice;
						}
						
						/* 查询运费 */
						$query_express="select price from weixin_commonshop_order_express_prices where isvalid=true and batchcode='".$batchcode_wzf."' limit 0,1";
						$result_express = mysql_query($query_express) or die('Query_express failed: ' . mysql_error());
						$o_express_price = 0;
						while ($row_express = mysql_fetch_object($result_express)) {
							$o_express_price = $row_express->price;
						...
						/* 查询运费End */
							$totalprice_2 = $totalprice_wzf+$o_express_price;
							$totalprice_2_sum += $totalprice_2;
					}										 				
					/*新查询未支付金额end */
					...
					$query2="select totalprice,batchcode from weixin_commonshop_orders wco where  paystatus=1 and  wco.isvalid=true and wco.user_id=".$user_id;
					$result2 = mysql_query($query2) or die('Query failed: ' . mysql_error());
					$totalprice_3 = 0;
				    while ($row2 = mysql_fetch_object($result2)) {
					   ...
					   $o_batchcode= $row2->batchcode;
						...
						/* 查询运费End */	
							$totalprice_3_last = $totalprice_3 + $o_express_price;	
							$totalprice_3_sum  +=$totalprice_3_last;		
					}
					/*查询已支付end*/
					...
					$query2="select createtime from weixin_commonshop_orders wco where   wco.user_id=".$user_id." order by id desc limit 0,1";
					$result2 = mysql_query($query2) or die('Query failed: ' . mysql_error());
					$createtime = 0;
				    while ($row2 = mysql_fetch_object($result2)) {
					...
					$totalprice_1_sum = round($totalprice_1_sum, 2);
					$totalprice_2_sum = round($totalprice_2_sum, 2);
					$totalprice_3_sum = round($totalprice_3_sum, 2);
					...
					$query3="select shop_id from weixin_neworder_orders where isvalid=true and customer_id=".$customer_id." and status=1 and orginid='".$weixin_fromuser."'";
					$result3 = mysql_query($query3) or die('Query failed: ' . mysql_error());
					$order_shop_id=-1;
					$shop_name="";
					while ($row3 = mysql_fetch_object($result3)) {
					   $order_shop_id = $row3->shop_id;
					   if($order_shop_id>0){
							$query4="select name from weixin_card_shops where isvalid=true and id=".$order_shop_id;
                ...
                <tr>
					...
					<td align="center">
						<a href="../../Order/order/order.php?customer_id=<?php echo $customer_id_en; ?>&search_class=3&user_id=<?php echo $user_id; ?>"><?php echo $totalprice_1_sum; ?></a>
					</td >	
					<td align="center">
						<a href="../../Order/order/order.php?customer_id=<?php echo $customer_id_en; ?>&search_class=9&user_id=<?php echo $user_id; ?>"><?php echo $totalprice_2_sum; ?></a>
					</td>
					<td align="center">	
						<a href="../../Order/order/order.php?customer_id=<?php echo $customer_id_en; ?>&search_class=8&user_id=<?php echo $user_id; ?>"><?php echo $totalprice_3_sum; ?></a>
					</td>
