...
<script type="text/javascript" src="js/shop.js"></script>
	<div class="r_nav">
		<ul>
			...
			<li class="cur"><a href="order3.php?customer_id=<?php echo $customer_id; ?>&status=-1">订单管理</a></li>
...
<div id="orders" class="r_con_wrap">
			<form class="search" id="search_form" method="post" action="order3.php?customer_id=<?php echo $customer_id; ?>&issearch=1">
			...
			订单号:<input type=text name="search_batchcode" style="width:100px;" value="<?php echo $search_batchcode; ?>" />&nbsp;
			...
			<tbody>
			   ...
			   ?>
                      <tr>
						   ...
						   <td><span id="order_totalprice_<?php echo $batchcode; ?>"><?php echo $t_totalprice; ?></span><?php if($expressfee>0){ echo "(".$expressfee.")"; } ?></td>
						   ...
						   <td>
							...
							<p><span onclick="if(!confirm('您确认要确认支付吗？确认后，表示订单已经支付，并且不能取消！'))return false; else goUrl('order3.php?batchcode=<?php echo $batchcode; ?>&op=confirm_pay&paystyle=<?php echo $paystyle; ?>&card_member_id=<?php echo $card_member_id; ?>&totalprice=<?php echo $t_totalprice ; ?>&paystyle=<?php echo $paystyle; ?>&pid=<?php echo $pid; ?>&user_id=<?php echo $user_id; ?>');" style="cursor:pointer">确认支付</span></p>
							<?php } ?>
							...
							<?php } ?>
							<?php if($status==0){ ?>
							<p><span onclick="if(!confirm('您确认要确认订单完成吗？确认后，表示订单已经完成，并且不能取消！'))return false; else goUrl('order3.php?batchcode=<?php echo $batchcode ?>&op=status&paystyle=<?php echo $paystyle; ?>&card_member_id=<?php echo $card_member_id; ?>&totalprice=<?php echo $t_totalprice ; ?>&paystyle=<?php echo $paystyle; ?>&pid=<?php echo $pid; ?>&user_id=<?php echo $user_id; ?>');" style="cursor:pointer">确认完成</span></p>
							<?php } ?>
							<?php if($sendstatus==3){ ?>
							<p><span onclick="if(!confirm('您确认要确认退货完成吗？确认后，表示该订单退货已经完成，并且不能取消！'))return false; else goUrl('order3.php?batchcode=<?php echo $batchcode ?>&op=status_back&paystyle=<?php echo $paystyle; ?>&card_member_id=<?php echo $card_member_id; ?>&totalprice=<?php echo $t_totalprice ; ?>&paystyle=<?php echo $paystyle; ?>&pid=<?php echo $pid; ?>&user_id=<?php echo $user_id; ?>');" style="cursor:pointer">退货完成</span></p>
							<?php } ?>
							<?php if($is_shopgeneral==0 or $is_generalcustomer==1){ ?>
							<p><span onclick="if(!confirm('您确认要删除此数据吗?删除后数据不能恢复!'))return false; else goUrl('order3.php?batchcode=<?php echo $batchcode ?>&op=del&card_member_id=<?php echo $card_member_id; ?>&totalprice=<?php echo $t_totalprice ; ?>');"  style="cursor:pointer">删除</span></p>
							<?php } ?>
						   </td>
					   </tr>
...
<script>
  ...
  function prePage(){
     ...
     document.location= "order3.php?pagenum="+pagenum+"&search_status="+search_status+"&begintime="+begintime+"&endtime="+endtime+"&pagesize="+pagesize;
  }
  ...
  function nextPage(){
     ...
     var begintime = document.getElementById("begintime").value;
	 var endtime = document.getElementById("endtime").value;
	 var url="order3.php?pagenum="+pagenum+"&search_status="+search_status+"&begintime="+begintime+"&endtime="+endtime+"&pagesize="+pagesize;
